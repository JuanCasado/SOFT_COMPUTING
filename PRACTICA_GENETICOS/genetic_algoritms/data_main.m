...
train = zeros(number_of_variables, ceil(size(data,2)/2));
test = zeros(number_of_variables, ceil(size(data,2)/2));
offset = 0;
for i = 1:size(data, 1)
    if variables_to_use(i)
        for j = 1:size(data, 2)
            if mod(j ,2)
                train(i-offset,(j+1)/2) = data(i,j)/normatization_data(i,j);
            else
                test(i-offset,j/2) = data(i,j)/normatization_data(i,j);
            end
        end
    else
        offset = offset + 1;
